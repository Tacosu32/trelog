...
開始、一時停止、再開、保存、目標達成、凍結自動使用、入力エラーの分岐で使う。
...
今日のスコアが目標スコア以上で、まだ今日の報酬を受け取っていない場合、凍結チケットを1枚付与する。
付与後は今日の日付を `claimedGoalRewardDates` に追加し、`trelog_state` に保存する。
...
`trelog_state.frozenDates` から凍結日のSetを返す。

### getContinuityDateSet()

記録日と凍結日を合わせた、継続扱いの日付Setを返す。

### hasContinuityBefore(dateText, continuityDates)

指定日より前に継続扱いの日付があるかを判定する。
凍結チケットをどこまで自動消費するか判断するために使う。

### consumeFreezeTicketsForMissedDays()

起動時に昨日以前の未記録日を確認し、継続が途切れそうな日を凍結チケットで補う。
チケットを使った場合は `frozenDates` と `freezeTickets` を更新し、`trelog_state` に保存する。

### calculateStreakDays()

記録が連続している日数を計算する。
記録日と凍結日を継続扱いにし、今日または昨日から過去へ連続日数を数える。
...
累計EXP、レベル、継続日数、凍結チケット枚数を画面に反映する。
...
初期値は凍結チケット2枚、凍結日なし、報酬受け取り済み日付なし。
...
`frozenDates` に含まれる凍結日を、履歴エリア下部に小さく表示する。

### saveTodayRecord()

現在の入力内容を取得し、入力チェック、簡易スコア計算、localStorage保存、今日の目標達成報酬、履歴再描画、各種ステータス更新を行う。
保存後はタイマーをリセットする。
...
起動時にlocalStorageから記録と状態を読み込み、凍結チケットの自動消費と今日の目標達成報酬の確認を行ったうえで、日付、履歴、今日のスコア、EXP、レベル、継続日数を画面に反映する。

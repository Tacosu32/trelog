...
### readCustomTrainerImage()

IndexedDBの `trainer_images` store から `customTrainerImage` を読み込む。

### writeCustomTrainerImage(file)

選択された画像ファイル本体、ファイル名、MIME type、保存日時をIndexedDBへ保存する。

### deleteCustomTrainerImage()

IndexedDBから `customTrainerImage` を削除する。
...
バックアップJSONへ含めるカスタム画像の有無、ファイル名、MIME type、保存日時を返す。

### buildBackupData()

`trelog_records`、`trelog_state`、`trelog_dev_scoring_config` とカスタム画像メタ情報を含むバックアップJSON用オブジェクトを作る。

# 2026-06-10 追記 トレーナー画像/一時停止コメント仕上げの関数

### getPresetTrainerImageCandidates(context, trainer) 変更

選択中プリセットトレーナーの状態別画像、同トレーナーの `default` 画像、既存public状態別画像、既存public通常画像の順で候補を返す。これにより `akari/rest.png` がない場合は `akari/default.png`、`trainer_rest.png`、`trainer_default.png` の順にフォールバックする。

### getTrainerImageCandidates(context) 変更

IndexedDBのカスタム画像候補を最優先し、その後に選択中プリセット画像、既存public画像、CSS仮表示へフォールバックする候補順を返す。カスタム画像優先仕様は維持する。

### startTimer() 変更

セッション開始/再開時のトレーナーコメントを固定文言ではなく `getTrainerLine()` から取得する。選択中トレーナーの `lines.session.resumed` などが反映される。

### pauseTimer() 変更

一時停止時のトレーナーコメントを固定文言ではなく `getTrainerLineGroup("session", TRAINER_LINES).paused` から取得する。選択中トレーナーの `lines.session.paused` を優先する。

### cancelSession() 変更

キャンセル後のホームコメントに、選択中トレーナーの `lines.rest.missedWarning` を使う。

### renderRecentRestEvent() 変更

直近の休憩チケット使用表示に、選択中トレーナーの `lines.rest.protected` を使う。

### renderRestDates() 変更

休憩日リストの補足情報に、選択中トレーナーの `lines.rest.usedDay` を使う。
...
`YYYY-MM-DD` 形式の日付文字列をローカル日付の `Date` に変換する。カレンダー表示で使う。

### isValidDateString(dateText)

`YYYY-MM-DD` 形式として扱える文字列かを判定する。状態補完とイベント補完で使う。

### getEarliestContinuityDate()

記録日と休憩日のうち、最も古い日付を返す。初回の日跨ぎ判定範囲を決めるために使う。

### calculateDailyScore(dateText)

指定日の筋トレログの `score` を合計する。カレンダーと目標達成表示で使う。

### isGoalReachedOnDate(dateText)

指定日の合計スコアが目標スコア以上かを返す。

### createRestTicketEvent(dateText, type)

休憩チケット使用履歴として `restTicketEvents` に保存するイベントを作成する。

### renderRecentRestEvent()

直近の休憩チケット使用状況をホーム画面に表示する。

### renderContinuityCalendar()

表示中の月について、記録日、休憩日、未記録日、今日、未来日を区別したカレンダーを描画する。

### moveCalendarMonth(offset)

継続カレンダーの表示月を前月または次月へ移動する。

### checkDateRollover()

起動後に日付が変わった場合、日付表示、日跨ぎ判定、統計表示を更新する。
...
現在はIndexedDBカスタム画像、選択中プリセット画像、既存public画像、CSS仮表示の順に返す。
...
起動時または日付更新チェック時に、前回チェック日の翌日から今日の前日までを古い順に確認する。
記録日でも休憩日でもない日があり、休憩チケットが残っていれば自動消費して `restDates` と `restTicketEvents` に追加する。
今日の未記録は当日中の挽回余地があるため休憩日にしない。
...
### setDateAsRecord(dateText)

デバッグ用に指定日を記録日にする。対象日の休憩日設定は外す。

### setDateAsRest(dateText, type)

デバッグ用に指定日を休憩日にする。対象日の記録は削除し、`restDates` と `restTicketEvents` に反映する。

### addPreviousDayToStreak()

デバッグ用に現在の連続記録の直前日へテスト記録を追加し、連続記録を1日伸ばす。

### removeLatestContinuityDay()

デバッグ用に直近の継続扱いの日を未記録状態に戻し、連続記録を1日減らす。

### clearYesterdayContinuity()

デバッグ用に昨日の記録と休憩日設定を削除し、昨日を未記録状態にする。

### runContinuityCheckManually()

デバッグ用に休憩チケット日跨ぎ判定を手動実行する。
...
# 2026-06-10 追記: リザルトEXP演出の関数

- `createResultSummary(record, beforeTotalExp, levelRewards, goalMarked)`
  - 保存前の累計EXPを受け取り、記録前後のレベル、記録前EXP、獲得EXP、記録後EXP、レベル進捗をリザルト用にまとめる。
- `buildResultExpAnimationSteps(result)`
  - 記録前EXPから記録後EXPまでのゲージ表示ステップを作る。
  - レベルアップ時は満タン表示と次レベル0%表示を挟み、複数レベルアップにも対応する。
- `animateResultExpGauge(result)`
  - `setTimeout` とCSS transitionでEXPゲージを段階的に更新する。
- `setResultExpGauge(totalExp, override)`
  - EXPゲージの幅とラベルを更新する。
- `clearResultAnimationTimers()`
  - リザルトを閉じたときに未完了のアニメーションタイマーを解除する。
- `getResultTrainerComment(result)`
  - 報酬獲得、レベルアップ、目標達成、通常記録などに応じてリザルト用コメントを返す。
# 2026-06-10 追記: トレーナー選択 v1 の関数

- `getSelectedTrainerId()`
  - `trelog_state.selectedTrainerId` を読み、存在しないIDの場合は `default` を返す。
- `getSelectedTrainer()`
  - 現在選択中の `TRAINER_PRESETS` のトレーナーデータを返す。
- `getTrainerLineGroup(groupName, baseLines)`
  - 既存の基本セリフに、選択中トレーナーの `lines[groupName]` を重ねて返す。
- `getPresetTrainerImageCandidates(context, trainer)`
  - 選択中プリセットの画像候補とpublicフォールバック候補を返す。
- `setPresetTrainerPreviewImage(imageElement, trainerId, context)`
  - 設定画面のトレーナー選択プレビュー用に、カスタム画像を使わずプリセット画像候補を表示する。
- `renderTrainerSelectionPanel()`
  - プリセットトレーナー選択UIを描画し、現在選択中のトレーナーを表示する。
- `updateTrainerSelectionPreview(trainerId)`
  - プルダウンで選んだトレーナーの名前、説明、口調、画像プレビューを更新する。
- `applySelectedTrainer()`
  - 選択中トレーナーIDを `trelog_state.selectedTrainerId` に保存し、各画面の画像とコメントを更新する。
- `getTrainerImageCandidates(context)`
  - 画像候補の順序を、カスタム画像、選択中プリセット画像、public画像、仮表示に整理する。
# 2026-06-10 追記 トレーナー選択 v1 仕上げ

### hasCustomTrainerImages()

IndexedDBから読み込まれたカスタムトレーナー画像差分が1件以上あるかを返す。トレーナー選択画面の説明表示や将来のカスタム画像優先設定に使える補助関数。

### renderTrainerImageSettings() 変更

カスタム画像差分数の表示に加え、トレーナー選択画面のカスタム画像優先説明を更新する。カスタム画像がある場合は画像が優先され、トレーナー切り替えでは名前、説明、口調、セリフが変わることを案内する。

### getHomeTrainerLine(context) 変更

選択中トレーナーの `lines.home` を優先し、今日記録済み、目標達成、目標まであと少し、休憩チケット状態、直近の自動消費、連続記録、時間帯に応じたコメントを返す。

### getTrainerLine(context) 変更

選択中トレーナーの `lines.session` を優先し、開始、10秒、30秒、60秒、一時停止、再開、高きつさ、低きつさ、目標達成見込み、回数入力に応じたコメントを返す。

### getResultTrainerComment(result) 変更

選択中トレーナーの `lines.result` を優先し、5レベル報酬、レベルアップ、今日の目標達成、高スコア、軽めの記録、高きつさ、通常記録に応じたコメントを返す。

### getTrainerImageCandidates(context) 変更なし

画像候補は、IndexedDBのカスタム画像、選択中プリセット画像、状況別public画像、通常public画像、CSS仮表示の順に維持する。プリセット専用画像が未配置でもpublic画像へフォールバックする。

# 2026-06-10 追記 設定画面アコーディオン関連

### setupSettingsSections()

設定画面の `details[data-settings-section]` を初期化する。`trelog_settings_sections_state` から前回の開閉状態を読み込み、各カテゴリの `open` 状態へ反映する。`toggle` イベントで開閉状態をlocalStorageへ保存する。

### loadSettingsSectionsState()

設定カテゴリの開閉状態をlocalStorageから読み込む。保存値がない場合やJSONとして読めない場合は、基本設定のみ開き、その他カテゴリを閉じる初期状態を返す。

### saveSettingsSectionsState()

設定カテゴリごとの開閉状態を `trelog_settings_sections_state` に保存する。この値は画面UI状態であり、バックアップ/復元対象には含めない。

### updateSettingsBasicSummary()

設定画面の基本設定カテゴリに、現在の今日の目標スコアとアプリバージョンを表示する。スコア調整やカウンセリング適用で目標スコアが変わった場合も `updateAllStats()` 経由で更新される。
# 2026-06-10 追記 カスタムトレーナー作成 v1

### normalizeCustomTrainerProfile(profile)

`trelog_state.customTrainerProfile` またはフォーム入力値を正規化する。名前が空の場合は無効として `null` を返す。

### getCustomTrainerPreset(profile)

保存済みのカスタムトレーナープロフィールを、既存の `TRAINER_PRESETS` と同じ形のトレーナーデータへ変換する。`lines.home/session/result/rest` はルールベース生成関数から作る。

### buildCustomTrainerLines(profile)

口調タイプ、励まし方、一人称、ユーザーの呼び方をもとに、ホーム、セッション、リザルト、休憩/継続系の主要コメントを返す。将来的にLLM生成へ差し替えやすいよう、既存の `getTrainerLineGroup()` から参照できる構造に合わせる。

### getCustomTrainerPreviewLines(profile)

ホーム未記録時、セッション開始時、一時停止時、リザルト通常時、目標達成時、レベルアップ時、休憩チケット使用時のプレビュー文を生成する。

### saveCustomTrainerProfile()

フォーム入力から `customTrainerProfile` を作成し、`trelog_state` に保存する。保存後はトレーナー選択UIとプレビューを再描画する。

### selectCustomTrainerProfile()

作成済みのカスタムトレーナーを `selectedTrainerId = "custom"` として選択する。未作成の場合は選択せずエラー表示する。

### resetCustomTrainerProfile()

`customTrainerProfile` を削除する。選択中トレーナーが `custom` の場合は既定トレーナーへ戻す。

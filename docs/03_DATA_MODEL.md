...
| evaluationProfile | 評価レベル | `standard` |

旧形式の `freezeTickets` と `frozenDates` が保存されている場合は、読み込み時に `restTickets` と `restDates` に移行する。
`evaluationProfile` がない既存データは、読み込み時に `standard` で補完する。
...
ユーザーの運動レベルに応じたスコア補正。
`trelog_state.evaluationProfile` に保存し、今後保存するログのスコア計算に使う。
過去ログのスコアは再計算しない。
...
| beginner | 初心者 | 1.2 |
| standard | 標準 | 1.0 |
| experienced | 慣れている | 0.85 |
| hard | 高負荷向け | 0.7 |

スコア計算は、種目係数、きつさ補正の後に評価プロファイル補正をかける。
